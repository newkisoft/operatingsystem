...
public:
    ...
    void FCFS(Jobs *alljobs, string *output, int numberOfJobs, int dumpTime, string outfile) {
        queue<Jobs> q;
        General general;
        int cnt;
    ...
    void RR(MemoryJobs *alljobs, string *output, int numberOfJobs, int quantom, int dumpTime, string outfile) {
        ...
        while (!q.empty()) {
            ...
            if (job.startTime <= currentTime) {
                ...
                string currStr = cache.pages[linePosition.pageNumber].lines[linePosition.lineNumber];
                ...
                cout << "\n";
             /*   if (strcmp(job.name.c_str(), "job2.txt") == 0) cout << "";
                if (strcmp(job.name.c_str(), "job3.txt") == 0) cout << "\t";
                cout << "--------------------------------------\n";
...
public:
    ...
    void RR(Jobs *alljobs, string *output, int numberOfJobs, int quantom, int dumpTime, string outfile) {
        queue<Jobs> q;
        int totalTime;
        int cnt = 0;
        Sort(alljobs, numberOfJobs);
...
int main(int argc, char** argv) {
    ...
    if (memory) {
        ...
        if (strcmp(argv[3], "FCFS") == 0) {
            ...
            outfile = argv[6];
            quantum = analysis.StringToDigit(argv[4]);
            analysis.RR(memoryJobs, output, numberOfJobs, quantum, dumpTime, outfile);
        }
        ...
        if (strcmp(argv[1], "FCFS") == 0)
            ...
            analysis.RR(alljobs, output, numberOfJobs, quantum, dumpTime, outfile);
        filetransfer.WriteFile("write", output, cnt);
    }
    return 0;
}
